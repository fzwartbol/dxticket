...
function MenuLink({ item }: MenuLinkProps) {
  ...
  if (!url) {
    return <span className="nav-link text-capitalize disabled text-white">{item.getName()}</span>;
  }

  if (item.getLink()?.type === TYPE_LINK_EXTERNAL) {
    return <a className="nav-link text-capitalize text-white" href={url}>{item.getName()}</a>;
  }

  return <Link to={url} className="nav-link text-capitalize text-white">{item.getName()}</Link>;
}
  ...
  return (
    <ul className={`navbar-nav col-12  ${page!.isPreview() ? 'has-edit-button' : ''}`}>
      ...
      { menu.getItems().map((item, index) => (
        <li key={index} className={`nav-item text-white ${item.isSelected() ? 'active' : ''}`}>
          <MenuLink item={item} />
        </li>
      )) }
    </ul>
